...
import com.suprnation.samples.logic.Logic.GetValue
...
object Logic {
  ...
  // Observations: registers the actor to receive state changes, the state changes will be tagged with the name wireName so we can correlate the received updates from within the actor ActorRef
  case class AddComponent(wireName: String, actor: ActorRef[IO])

  // Observations: A state change happened in the upstream wire.  The name here is the name used during registration.
  case class StateChange(name: String, state: Boolean)

  // Observations: Requests the current value.
  ...
  object LogicCircuits {
          ...
          case s: Boolean =>
            currentStateRef.get.map(_ != s).ifM(
              for {
                ...
                _ <- associations.toList.parTraverse_ { case (ref: ActorRef[IO], name: String) =>
    ...
    def reducer(reduceFn: (Boolean, Boolean) => Boolean, delay: Duration)(input0: ActorRef[IO], input1: ActorRef[IO], output: ActorRef[IO]): IO[Actor[IO]] = {
      for {
        in0 <- Ref[IO].of[Option[Boolean]](None)
        in1 <- Ref[IO].of[Option[Boolean]](None)
      } yield new Actor[IO] {
    ...
    def halfAdder(a: ActorRef[IO], b: ActorRef[IO], s: ActorRef[IO], c: ActorRef[IO]): Actor[IO] = {
      new Actor[IO] {
        override val preStart: IO[Unit] = for {
          ...
          d <- context.actorOfWithDebug(PropsF[IO](wire(false)), "d")
          e <- context.actorOfWithDebug(PropsF[IO](wire(false)), "e")
          _ <- context.actorOfWithDebug(PropsF[IO](or(a, b, d)), "Or")
          _ <- context.actorOfWithDebug(PropsF[IO](and(a, b, c)), "And")
          _ <- context.actorOfWithDebug(Props[IO](inverter(c, e)), "Inverter")
          _ <- context.actorOfWithDebug(PropsF[IO](and(d, e, s)), "And2")
        } yield ()
      }
    }
